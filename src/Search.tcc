...
bool CompactedDBG<U, G>::search(const vector<string>& query_filenames, const string& out_filename_prefix,
    ...
    if (verbose) cout << "query_name," << "unitig_head_kmer," << "unitig_tail_kmer," << "mapped_head_kmer" << endl;
    ...
    if (nb_threads == 1){
        ...
        while (fp.read(s, file_id)){
            ...
            if (get_nb_found_km || get_ratio_found_km) { // Search for all k-mers but return only the number of found k-mers
                ...
                // print out locations of each pseudoalignment 
                if (verbose) {
                    for (const auto& p : v){
                        if (p.first == 0){
                            cout << query_name << "," << p.second.getUnitigHead().toString() << "," << p.second.getUnitigTail().toString() << "," << p.second.getMappedHead().toString() << endl; 
                        }
                    }
                }
